...
# ==================================================
# Load ANN model and scaler (cached globally)
# ==================================================
@st.cache_resource
def load_ann_model():
    """Load the trained ANN model"""
    # Try standalone Keras first (supports newest .keras ZIP format)
    try:
        import keras
        model = keras.models.load_model("mumbai_ann_model_fixed.keras")
        return model
    except Exception as e:
        st.warning(f"Standalone Keras load failed, trying TensorFlow: {e}")
...
st.markdown("<div class='model-badge'>🧠 Powered by Artificial Neural Network (ANN)</div>", unsafe_allow_html=True)

# ==================================================
# Tabs
# ==================================================
tab1, tab2, tab3 = st.tabs(["⚡ Predict Price", "🗺️ Explore Map", "🏘️ Neighbourhood Clusters"])
...
with tab1:
    with st.expander("📌 Enter Property Details", expanded=True):
        ...
        with col1:
            bhk = st.slider("🛏️ Number of BHK", min_value=1, max_value=3, value=2, step=1)
    ...
    # Create feature vector (must match training order: bhk, area, age)
    ...
    # Set age (label encoded: 0=Unknown, 1=New, 2=Resale)
    if age == "New":
    ...
    # Set region (one-hot encoded)
    if region != "Any / Not Sure" and region in regions:
    ...
    if st.button("🚀 Predict Price", use_container_width=True):
        try:
            ...
            # Optional: Show prediction confidence or range
            with st.expander("ℹ️ Prediction Details"):
                st.write(f"**Input Features:**")
                st.write(f"- Area: {area} sqft")
                st.write(f"- BHK: {bhk}")
                st.write(f"- Age: {age}")
                st.write(f"- Region: {region if region != 'Any / Not Sure' else 'Not specified'}")
                st.write(f"\n**Model:** Artificial Neural Network with Adam Optimizer")
                
        except Exception as e:
            st.error(f"❌ Prediction failed: {e}")
            st.info("Please ensure the model file 'mumbai_ann_model.h5' and 'scaler.pkl' are in the same directory.")
