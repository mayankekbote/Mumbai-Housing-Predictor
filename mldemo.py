...
@st.cache_resource
def load_model():
    with open("mumbai_demo_Scaling (1).pkl", "rb") as f:
        rf, scaler = pickle.load(f)  # unpack the tuple
    return rf, scaler
...
with tab1:
    ...
    if st.button("🚀 Predict Price", use_container_width=True):
        try:
            prediction_scaled = model.predict([input_data])  # returns array of shape (1,)

    # Inverse-transform to get actual price
            prediction = scaler.inverse_transform(prediction_scaled.reshape(1, -1))[0][0]  # scalar
...
# -----------------------------
# Tab 3: Resale vs New Flats Map
# -----------------------------
with tab3:
    st.markdown("## 🏘️ Resale vs New Flats in Mumbai")
    st.caption("🔵 Blue = Resale dominates | 🟢 Green = New dominates | Circle size ∝ Number of flats")
    ...
    # Count resale vs new per region
    resale_counts = mumbai[mumbai["age"] == 2].groupby("region").size().reset_index(name="resale_count")
    new_counts = mumbai[mumbai["age"] == 1].groupby("region").size().reset_index(name="new_count")

    # Merge counts with coordinates
    counts = pd.merge(df, resale_counts, on="region", how="left").merge(new_counts, on="region", how="left")
    counts.fillna(0, inplace=True)
    counts["total_flats"] = counts["resale_count"] + counts["new_count"]
    counts["dominant"] = np.where(counts["resale_count"] > counts["new_count"], "Resale", "New")

    # Apply region filter
    if selected_regions:
        counts = counts[counts["region"].isin(selected_regions)]

    # Create map
    m2 = folium.Map(location=[19.0760, 72.8777], zoom_start=10, tiles=map_style)

    # Add circles
    for _, row in counts.iterrows():
        color = "blue" if row["dominant"] == "Resale" else "green"
        radius =10 # radius ∝ sqrt(total flats)
        popup_html = f"""
            <div style='font-size:14px'>
            <b>{row['region'].title()}</b><br>
            🔵 Resale: {int(row['resale_count'])}<br>
            🟢 New: {int(row['new_count'])}<br>
            📦 Total flats: {int(row['total_flats'])}
            </div>
        """
        folium.CircleMarker(
            location=[row['latitude'], row['longitude']],
    ...
    st_folium(m2, width='100%', height=700, key="resale_vs_new_map")
